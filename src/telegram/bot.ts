...
import { IParseSuccess, IParseError } from '../parser/parse.type';
...
const sendMarkdownTemplate = async (chat_id: number | string, data: IParseSuccess): Promise<void> => {
	await bot.api.sendMessage(chat_id, getMarkdownAfterParse(data), {
		parse_mode: 'HTML'
	});
}

const sendMediaGroup = async (chat_id: number | string, data: IParseSuccess): Promise<void> => {
	const promises = []
	const chunkSize = 10;
...
const parseInfo = async (vin: string): Promise<IParseSuccess | IParseError | null> => {
